#' @title Plot TS Chips (RGB version) adapted for Sentinel-2 downloaded in google earth engine
#' 
...
#' 
#' @author Ben DeVries (adaptation by Hadi)
#' 
#' @import zoo
#' @import raster
#' @export
...
tsChipsRG_Sentinel2 <- function(xr, xg, xb, loc, start = NULL, end = NULL, buff = 17, percNA = 20, nc = 3, nr = 3, plot = FALSE, exportChips = FALSE, exportZoo = FALSE, textcol = "white", plotlabs = c('red', 'green', 'blue'), ...) {
  ...
  # get sceneinfo
  # s <- getSceneinfo(names(x$R))                                  # Exclude
  
  # set z-dimensions of each brick
  # x <- lapply(x, FUN=function(x) setZ(x, s$date))              # Exclude, input bricks already have Z attribute 
  ...
  # # start and end dates             # Exclude for now, need to insert setZ based on Sentinel-2 id
  # if(!is.null(start)){
  #   start <- as.Date(start)
